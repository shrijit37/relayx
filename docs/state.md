...
| Fact | Value |
| ---- | ----- |
| rust_tests_total | 329 |
| rust_tests_gateway | 84 |
| rust_tests_protocol_core | 133 |
| rust_tests_workflow_schema | 16 |
| rust_tests_workflow_runtime | 92 |
| rust_tests_mock_upstream | 4 |
| web_tests | 38 |
| control_plane_tests | 51 |
| page_routes | 15 |
<!-- END CANONICAL FACTS -->
## Status
...
**Test count (Phase 6, now 329 Rust after egress-control work): Rust 329 passing, zero failures.** Baseline 267 → +27 (control-plane e2e, gateway `/run`, SQL injection defense-in-depth), then −5 from the 2026-09-16 ponytail cleanup (removed dead node-extension and provider machinery), then +17 from extension core (ExtensionRegistry, worker RPC transport, extension integration tests), then +4 from the extension-core gap work (validator-before-executor, fail-closed validation, serde roundtrip, registry snapshots), then +18 from egress-control (rotation roundtrip, retry-on-429, e2e 429 rotation, LaneClient connector unit tests, WireLane serde roundtrip, masked-egress pool builder tests). **Control plane: 51 integration tests** (api 18, publish 7, sql-injection 4, catalog 17, reaper 5) against a real Postgres 16 + in-process mock gateway. **Frontend: tsc clean, 38 tests (workflow-serializer + run-state + WorkflowBuilder interaction tests), production build clean.**
...
- **Rust: 329 tests** (workspace, zero failures).
- **Control plane: 51 tests** (api 18, publish 7, sql-injection 4, catalog 17, reaper 5) against a real Postgres 16 + in-process mock gateway.

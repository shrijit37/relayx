...
**Test count (Phase 6, now 345 Rust after egress-control fail-closed + correctness fixes): Rust 345 passing, zero failures.** Baseline 267 → +27 (control-plane e2e, gateway `/run`, SQL injection defense-in-depth), then −5 from the 2026-09-16 ponytail cleanup (removed dead node-extension and provider machinery), then +17 from extension core (ExtensionRegistry, worker RPC transport, extension integration tests), then +4 from the extension-core gap work (validator-before-executor, fail-closed validation, serde roundtrip, registry snapshots), then +18 from egress-control (rotation roundtrip, retry-on-429, e2e 429 rotation, LaneClient connector unit tests, WireLane serde roundtrip, masked-egress pool builder tests), then +8 from egress-control correctness fixes (wire-compat retry/fallback defaults, gateway egress fail-closed rejection, fallback shared-client e2e, default-429 rotation e2e), then +8 from fail-closed egress hardening (masked `/run` resolves the lane pool, masked-without-proxy rejected at pool build and from_lane, runtime client resolution requires per-lane pools, unknown-egress rejection). **Control plane: 55 integration tests** (api 21, publish 8, sql-injection 4, catalog 17, reaper 5) against a real Postgres 16 + in-process mock gateway. **Frontend: tsc clean, 43 tests (workflow-serializer + run-state + WorkflowBuilder interaction tests), production build clean.**
...
- **Rust: 345 tests** (workspace, zero failures).
- **Control plane: 55 tests** (api 21, publish 8, sql-injection 4, catalog 17, reaper 5) against a real Postgres 16 + in-process mock gateway.
- **Frontend: 43 tests** (workflow-serializer + run-state reducer + WorkflowBuilder interaction tests) run via `bun test` with happy-dom + @testing-library/react. Interaction tests guard against dead-UI regressions (stubbed `RunPanel`, unwired toolbar buttons).

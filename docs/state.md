...
| Fact | Value |
| ---- | ----- |
| rust_tests_total | 349 |
| rust_tests_gateway | 96 |
| rust_tests_protocol_core | 133 |
| rust_tests_workflow_schema | 22 |
| rust_tests_workflow_runtime | 94 |
| rust_tests_mock_upstream | 4 |
| web_tests | 48 |
| control_plane_tests | 56 |
| page_routes | 15 |
<!-- END CANONICAL FACTS -->
## Status
...
**Phase 1 extension-core gaps FILLED** — Custom nodes now execute end-to-end: publish → gateway → worker RPC. Key changes: (1) `CustomConfig.kind` renamed to `ext_kind` on the wire to fix a serde collision with the `NodeConfig` tag, (2) validators run before the executor (fail-closed on validation error), (3) extension registry threaded through `compile_workflow_with_lanes` and `PublicationState`, (4) control plane collects distinct `ext_kind` values into `WireSnapshot.extensions` for observability. Test count: 329 Rust (+15: LaneClient connector unit tests, WireLane serde roundtrip, masked-egress pool builder tests), 51 control-plane (+3: extension collection).
...
**Test count (Phase 6, now 349 Rust after egress-control fail-closed + correctness fixes): Rust 349 passing, zero failures.** Baseline 267 → +27 (control-plane e2e, gateway `/run`, SQL injection defense-in-depth), then −5 from the 2026-09-16 ponytail cleanup (removed dead node-extension and provider machinery), then +17 from extension core (ExtensionRegistry, worker RPC transport, extension integration tests), then +4 from the extension-core gap work (validator-before-executor, fail-closed validation, serde roundtrip, registry snapshots), then +18 from egress-control (rotation roundtrip, retry-on-429, e2e 429 rotation, LaneClient connector unit tests, WireLane serde roundtrip, masked-egress pool builder tests), then +8 from egress-control correctness fixes (wire-compat retry/fallback defaults, gateway egress fail-closed rejection, fallback shared-client e2e, default-429 rotation e2e), then +8 from fail-closed egress hardening (masked `/run` resolves the lane pool, masked-without-proxy rejected at pool build and from_lane, runtime client resolution requires per-lane pools, unknown-egress rejection), then +2 from proxy egress resolution (masked proxy route fail-closed, direct lane shared-client wrap), then +2 from parity A+C (fallback per-entry protocol + input/output editor-metadata wire-compat round-trips). **Control plane: 56 integration tests** (api 22, publish 8, sql-injection 4, catalog 17, reaper 5) against a real Postgres 16 + in-process mock gateway. **Frontend: tsc clean, 48 tests (workflow-serializer + run-state + WorkflowBuilder interaction tests), production build clean.**
...
Execution engine (81 tests: unit + integration + extension-proof + protocol + context capabilities). Provides:
...
- `compile_workflow(workflow, ctx)` — schema + lane + extension-kind validation (including compile-time rejection of lane-less LLM nodes when 0 or multiple lanes exist)
...
- Real node implementations: **LLM** (real provider calls + incremental SSE streaming), Transform, Condition, Router, Fallback, Retry, **Custom** (out-of-process worker RPC via `ExtensionRegistry`)
- `ExecutionContext` — lane registry, lane-aware client resolver (`AsLaneClient`), snapshot identity, execution metadata (snapshot_version/plan_hash), milestone reporter, **extension registry**
- `ExtensionRegistry` + `ExtensionSpec` + `ExtensionValidator`/`ExtensionExecutor` traits — maps custom node kinds to validator + executor; custom nodes execute out-of-process via Unix domain socket worker RPC (`extension::worker::UnixSocketExecutor`)
- `RuntimeSnapshot` / `RuntimeSnapshotBuilder` — immutable bundle of plans (each carrying its ACTIVE workflow version) + lanes + extension specs (kind + version metadata); published atomically via `SnapshotPublisher`/`InMemoryPublisher` (ArcSwap); provider registry removed 2026-09-16 (never populated)
...
- **Rust: 349 tests** (workspace, zero failures).
- **Control plane: 56 tests** (api 22, publish 8, sql-injection 4, catalog 17, reaper 5) against a real Postgres 16 + in-process mock gateway.
- **Frontend: 48 tests** (workflow-serializer + run-state reducer + WorkflowBuilder interaction tests) run via `bun test` with happy-dom + @testing-library/react. Interaction tests guard against dead-UI regressions (stubbed `RunPanel`, unwired toolbar buttons).
...
- **Custom node extension core** — `ExtensionRegistry` + `ExtensionExecutor` trait + Unix socket worker RPC transport implemented; no real extensions registered yet. Custom nodes fail at runtime with a clear typed error when no registry is present. Extensions must be registered at gateway startup or via a future control-plane extension registration endpoint.
...
- custom extensions execute out-of-process (worker RPC, never dlopen)

...
**Phase 1 extension-core gaps FILLED** — Custom nodes now execute end-to-end: publish → gateway → worker RPC. Key changes: (1) `CustomConfig.kind` renamed to `ext_kind` on the wire to fix a serde collision with the `NodeConfig` tag, (2) validators run before the executor (fail-closed on validation error), (3) extension registry threaded through `compile_workflow_with_lanes` and `PublicationState`, (4) control plane collects distinct `ext_kind` values into `WireSnapshot.extensions` for observability. Test count: 311 Rust (+4: validator-before-executor, validator-fail-blocks, serde roundtrip, specs_snapshots), 51 control-plane (+3: extension collection).
...
**Test count (Phase 6, now 311 Rust after the extension-core gap work): Rust 311 passing, zero failures.** Baseline 267 → +27 (control-plane e2e, gateway `/run`, SQL injection defense-in-depth), then −5 from the 2026-09-16 ponytail cleanup (removed dead node-extension and provider machinery), then +17 from extension core (ExtensionRegistry, worker RPC transport, extension integration tests), then +4 from the extension-core gap work (validator-before-executor, fail-closed validation, serde roundtrip, registry snapshots). **Control plane: 51 integration tests** (api 18, publish 7, sql-injection 4, catalog 17, reaper 5) against a real Postgres 16 + in-process mock gateway. **Frontend: tsc clean, 38 tests (workflow-serializer + run-state + WorkflowBuilder interaction tests), production build clean.**
...
- **Rust: 311 tests** (workspace, zero failures).

...
import type { CanonicalConfig, CanonicalNode } from "@/lib/workflow/nodes";
import { getNodeDefinition, type FieldDef } from "@/lib/workflow/node-definitions";
...
function Field({
  ...
  const refOptions =
    def.reference === "lanes" ? laneOptions : def.reference === "providers" ? providerOptions : def.options ?? [];
  const isRefSelect = (def.reference === "lanes" || def.reference === "providers" || def.reference === "models") && refOptions.length > 0;
  ...
  return (
    <label className="block">
      ...
      {def.type === "boolean" ? (
        ...
        <select value={optValue} onChange={(e) => onOpt(e.target.value)} className={cn(inputCls, "appearance-none")}>
      ...
      <aside className={cn("flex min-h-0 flex-col bg-panel", className)}>
        ...
        <div className="flex flex-1 items-center justify-center p-6 text-center">
          <p className="max-w-[190px] text-[11px] leading-relaxed text-muted-foreground">
            Select a node to edit its configuration. Runtime health/per-node metrics are not shown — there is no
            per-node telemetry backend yet.
          </p>
        </div>
      </aside>
    );
  ...
  return (
    <aside className={cn("flex min-h-0 flex-col bg-panel", className)}>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        <Group label={`Configuration · ${def?.schemaVersion ? `v${def.schemaVersion}` : "v1"}`}>
          <div className="space-y-2.5">
            ...
            {def?.fields.length === 0 ? (
              <p className="text-[11px] text-muted-foreground">This node has no configuration fields.</p>
            ) : null}
          </div>
        </Group>
              ...
              {nodeIssues.map((i, idx) => (
                <div
                  ...
                  className={cn(
                    ...
                    i.severity === "error" ? "border-fail/40 bg-fail/8 text-fail" : "border-warn/40 bg-warn/8 text-warn",
                  )}
                >
        ...
        {def?.note ? (
          <Group label="Runtime">
            <p className="text-[11px] leading-relaxed text-muted-foreground">{def.note}</p>
          </Group>
        ) : null}
...
function valueOf(config: CanonicalConfig, name: string): unknown {
  switch (config.kind) {
    case "llm": return config.config[name as keyof typeof config.config];
    case "router": return name === "strategy" ? config.strategy : undefined;
    case "transform": return name === "operation" ? config.operation : undefined;
    ...
    case "retry": {
      if (name.startsWith("target.")) return config.target[name.slice(7) as keyof typeof config.target];
...
function applyField(config: CanonicalConfig, name: string, value: unknown): void {
  switch (config.kind) {
    case "llm": (config.config as unknown as Record<string, unknown>)[name] = value; break;
    case "router": if (name === "strategy") config.strategy = value as never; break;
    case "transform": if (name === "operation") config.operation = value as never; break;
    case "condition": (config.condition as unknown as Record<string, unknown>)[name] = value; break;
    case "mcp": (config.tool as unknown as Record<string, unknown>)[name] = value; break;
    case "skill": (config.skill as unknown as Record<string, unknown>)[name] = value; break;
    case "fallback": if (name === "rounds") config.fallback.rounds = (Number.isFinite(Number(value)) ? Number(value) : 1); break;
    case "retry":
      if (name.startsWith("target.")) (config.target as unknown as Record<string, unknown>)[name.slice(7)] = value;
